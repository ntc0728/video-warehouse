...
| `demos/demo-iptv-channel-list-osd-2026-09-10.html` | **IPTV 播放页 chrome 分析与提案（待拍板，未落地代码）**：① 频道列表一/二级宽度——现状硬 50/50 实测一级空转 97px（44%）、二级名称只剩 122.2px（最长名 157px 截断 34.9px，375 下只剩 54.5px）；4 个方案并排（现状 / **A★ 一级按内容派生定值 132px、≤479 走 116px 窄屏档、二级吃剩余** / B 32% 比例档（375 下分类名截断 22.6px）/ C 面板换 episode-panel-w 仍 50/50），附「一级内容需要」实测标尺；② OSD 栏宽度——现状 token 在 420–1158px 视口区间被 max-width 吃掉恒贴边（768 占 97.5%、1024 占 98.0%），三代对比条形图 + 建议 `clamp(320px,78vw,1400px)`；③ OSD 内部布局——现状左翼 169.4 / 右翼 113.6 致控件行右偏 27.9px，对比 现状 / **建议 v1★ 右翼改用 --layout-channel-num-w（偏移归 0）+ 三行并两行** / 提议 v2（号与台名同排，取舍：台名可见宽 130.6→101px）；6 个真实视口切换 + 常规/超长频道名切换，token 全部由运行中播放器实测 getComputedStyle 读回 | 2026-09-10 |
