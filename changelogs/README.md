...
| `demos/demo-widescreen-overhaul-2026-09-10.html` | **大屏 UI 整改四议题提案台（待拍板，未落地代码）**：① 首页大屏——A 现状 / B 内容区两栏（Hero 全宽 + 左栏热度榜 sticky）/ C 通体两栏 / D Hero 右栏承载热度榜★，附「顶栏与内容区对齐」「顶部过渡带」两个可叠加开关；② Browse 左栏紧凑——A 现状（1440 下栏宽 124px、chip 61×32、栏高 836px）/ B 紧凑 2 列★（栏宽 148px、chip 73×26、栏高 694px，−17%）/ C 自适应列数 / D 分组折叠（栏高 248px）；③ 左栏视觉语言统一——A 现状（无边框无底）/ B 右侧 1px 边框★ / C 浅底面板 / D 浅底+右边框，同一样本页跑四种；④ Detail/Person 大屏——A 现状（1920 下 hero 562 / Person 450px）/ B 降高（397/272px，−29%）/ C1 右栏=文本类信息★（类型标签+基础信息 KV+发行，实测恰好填满 hero 高、**不需要滚动**；演员/简介/剧照下移通栏）/ C2 右栏+简介（6 行截断）/ C3 右栏+剧照 2×2 小图 / C4 右栏全文+栏内滚动（后三档内容超出 hero 高，靠右栏滚动兜底，两栏下沿始终齐平）。**Demo 技术**：用 CSS `@container` 模拟 1280/1440/1920/2560 视口（项目 vw→cqw、vh→`--sim-h`×比例），可按宽度与主题（浅/暗）实时切换；60 个「宽度×议题×方案」组合经 DOM 实测无横向溢出、无裁切 | 2026-09-10 |
