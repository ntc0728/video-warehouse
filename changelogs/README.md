...
| `demos/demo-category-quick-access-a2-2026-09-06.html` | CategoryQuickAccess 方案 A2 拓展（**待审阅，未落地**）：频道导航 + 「全部分类」跳 browse；点击 chip 下方展开「genre 分类 + 最热门搜索值（热度=popularity）」面板，点 genre chip 实时切榜；默认分类热度榜（trending/all/day 聚合 Σpopularity + 各分类 top1 搜索值）；**全部数据运行时调真实 TMDB API（zh-CN），双域名回退（api.tmdb.org 备用域名优先，主域名部分网络不可达），请求日志可视** | 2026-09-06 |
