...
| `demos/demo-record-filter-layout-2026-09-06.html` | 收藏/历史页筛选面板布局 5 方案对比（**待拍板，未落地**）：A 现状基线（更多筛选折叠）/ B 面板常驻（用户提案：去按钮 + RecordFilterPanel animate-fade-in 恒显示）/ C ★推荐——状态 chips 常驻并入顶栏 + 排序收纳为胶囊弹出层（chips 交互保留）/ D 全行化一行塞满 / E 附加项——清空操作降级进「⋯」溢出菜单 + IPTV tab 下隐藏无效状态 chips；chips/弹层可交互，card 不动 | 2026-09-06 |
