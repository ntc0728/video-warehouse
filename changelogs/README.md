...
| `demos/demo-browse-topbar-2026-09-23.html` | **Browse 顶栏四方案对比（待拍板，未落地顶栏代码）**：首行=左模式胶囊（智能检索｜直链搜索）+ 右类型 7 档分段·分隔线·排序 3 档·共 N 条（直链=结果数+源 badge）；问题基线 height:32 硬等高+stretch；**A★推荐 光学居中**（各控件 pad 推导、行内 center、计数不参与等高）/ B 分组松弛（stab 描边胶囊、count 降 text-xs）/ C 极简下划线；顶栏可切智能/直链；同文件示意左栏无滚动条（对应已落地的 FilterBar 行壳 + rail overflow:visible） | 2026-09-23 |
