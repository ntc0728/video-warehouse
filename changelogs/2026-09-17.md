...
### 改动
> ⚠️ `android/` 被 `.gitignore` 忽略，CI 靠 `cap add` + `cap sync` 重建 `build.gradle`，手动改会被覆盖。
> 故「依赖声明」不放进 `android/app/build.gradle`，而在补丁脚本（cap sync 之后执行）里注入。

1. `scripts/patch-android-dlna.ps1`：新增第 3 步，在 `cap sync` 重建的 `android/app/build.gradle` 中
   幂等注入 `implementation "androidx.media:media:1.7.0"`（锚定 `androidx.appcompat` 依赖行，已含则跳过）。
2. `scripts/android-dlna-patch/java/.../MediaService.java`：3 行 import 由
...
- 依赖注入点在 `patch-android-dlna.ps1`（CI 与本地 `build-android.ps1` 均在 cap sync 后调用），随平台重建后仍存在。
- 本地/CI Android 构建（gradle assembleRelease）需 Android SDK + 联网拉取 androidx.media:1.7.0；本沙箱未配置，交由下一次 CI（推送触发 release-please.yml build-apk）验证。
