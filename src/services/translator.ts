...
function generateSign(text: string, appId: string, salt: number, key: string): string {
  const signStr = appId + text + salt + key;
  return md5(signStr);
}

export async function translate(options: TranslateOptions): Promise<string> {
  const { text, from = 'auto', to, appId, key } = options;

  if (!appId || !key) {
    console.warn('Translation API credentials not configured');
    return text;
  }
  ...
  try {
    ...
    const response = await request<BaiduTranslateResponse>(BAIDU_TRANSLATE_API, {
      method: 'POST',
