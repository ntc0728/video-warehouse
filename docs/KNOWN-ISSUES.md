...
## 13. [已知 · 未修 · 待办] spacing token 曲线 bug：桌面恒卡 MIN（2026-09-03 列入待办，暂不处理）

**状态**：⚠️ 已知（2026-09-03 晚分析定位；用户当日拍板**不修复、列入待办**，勿擅自动手）。

**现象**：非 TV 桌面（≥768px）`--space-*` 全部解析为 MIN 下限——`--space-lg` 桌面 = **12px**（设计契约 28px）、`--space-xl` = 16px（契约 40px）等，≥1920 大屏不随视口放大。仅 TV 端正常（独立放大曲线 28→56px）。

**根因**：`variables.css:166-177` 11 条 spacing token 的 **vw 系数漏乘 100**（写作 `0.0407vw` 的 px-per-px 裸值，本应 `4.07vw`）；且 rem 常数项按 16px 基准书写，实际根字号 14px（`index.css:258` `html{font-size:var(--text-base)}`），两者叠加导致中间项 ≥768px 不爬升，恒取 MIN。曾按授权修复（系数 ×100）使全站间距整体放大，观感变化过大，**用户指示回退**（恢复原状，见 changelogs/2026-09-03.md「token 曲线修复被用户否决并已回退」）。

**影响**：卡片网格 gap（本轮统一为 `var(--space-lg)`）桌面仅 12px；≥1920 大屏 8–10 列 + 12px gap 明显偏挤；页面左右 padding（`.page-padding` 用 space-lg）桌面仅 12px。

**修复方向（待办，勿执行）**：二选一——
1. 修 token 曲线（须连同全站观感复核，改动面大，已否决一次）；
2. **网格单独补大屏 gap 放大档**（风险最小，推荐）：保持 `gap:var(--space-lg)` 基准，另在 ≥1920/2560/3840 对卡片网格加放大档，复用旧 `--grid-gap` ≥1920 `clamp(20px,1rem+0.5vw,36px)` 手法，不碰全局 token。
...
| 规则 | 实际算出的值 | 效果 |
| --- | --- | --- |
| `.up-channel-group-active::before`（一级活跃左竖条） | `rgb(0,0,0)` | 完全不可见（1.84 × 16.5px 黑条压在黑面板上） |
| `.up-channel-item-active`（二级活跃左边框） | `3px rgb(0,0,0)` | 完全不可见 |
| `.up-channel-group-active .up-channel-group-count` | `color(srgb 0 0 0 / 0.12)` | 比未选中态的 `rgba(255,255,255,.06)` **更暗** —— 选中反而更弱 |
| `.up-channel-item-active .up-channel-item-num` | `color(srgb 0 0 0 / 0.65)` | 不可见（序号本该高亮） |

目前唯一生效的活跃线索是行底色 `rgba(255,255,255,.12)` + 字重加粗。

**根因**：播放器页面跑在应用默认的**浅色主题**下，`--color-primary` 在 `variables.css` 浅色
`:root` 是 `#000`（暗色主题块才是 `#fff`）；而频道列表面板本身是 `rgba(0,0,0,.60)`
压在视频黑场上 —— 黑色 primary 与面板同色，所有强调规则退化为不可见。

**修复方向**：播放器作用域内覆写 primary，例如
`.up-universal-player { --color-primary: #3b82f6; --color-primary-hover: #60a5fa; }`
